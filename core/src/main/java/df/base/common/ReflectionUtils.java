...
abstract public class ReflectionUtils {
    ...
    public static <T> T instantiate(Constructor<T> constructor, Object... arguments) {
        ...
        try {
            ...
            throw new RuntimeException("UNABLE TO CREATE INSTANCE OF: " + constructor.getDeclaringClass().getName(), e);
        }
    ...
    public static Constructor<?> findFirstConstructor(Class<?> clazz, Class<?>... types) {
        ...
        if (constructor == null) {
            throw new RuntimeException(clazz.getName() + ": CONSTRUCTOR WITH (" + Arrays.toString(types) + ") PARAMETERS NOT FOUND");
        }
    ...
    public static Constructor<?> findFirstAnnotatedConstructor(Class<?> clazz, Class<? extends Annotation> annotation) {
        ...
        if (!iterator.hasNext()) {
            throw new RuntimeException("CANNOT FIND APPROPRIATE ANNOTATED CONSTRUCTOR FOR CLASS: " + clazz.getName() + " WITH @" + annotation.getName());
        }
